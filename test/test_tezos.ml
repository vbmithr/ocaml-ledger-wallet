...
let test_git_commit () =
  with_connection (fun h -> get_git_commit h >>| ignore)
...
let test_sign h curve =
  ...
  | Secp256k1 -> begin
      let out =
        Secp256k1.Key.read_pk
          secp256k1_ctx (Cstruct.to_bigarray pk) >>= fun pk ->
        (* Remove parity info *)
        Cstruct.(set_uint8 signature 0 (get_uint8 signature 0 land 0xfe)) ;
        Secp256k1.Sign.read_der
          secp256k1_ctx (Cstruct.to_bigarray signature) >>= fun signature ->
        let msg = Secp256k1.Sign.msg_of_bytes_exn msg_ba in
        Secp256k1.Sign.verify secp256k1_ctx ~pk ~msg ~signature in
      match out with
...
let test_sign () =
  with_connection
    (fun h -> test_sign h Secp256r1 >>= fun () -> test_sign h Secp256r1)
